...
let spin = function(io, user){
	...
	HU.findOne({game:'mini3cay', type:cuoc, red:true}, {}, function(err, data) {
		...
		if (bo3 && bo3_a === 0) {
			...
			Mini3Cay_red.create({'uid': user.id, 'win': an, 'bet': cuoc, 'type': 6, 'kq': ketqua, 'time': new Date()});
			io.sendInHome({pushnohu:{title:'MINI 3 CÂY', name:user.name, bet:an}});
		}else if (Day && dongChat) {
			an = cuoc*30;
			Mini3Cay_red.create({'uid': user.id, 'win': an, 'bet': cuoc, 'type': 5, 'kq': ketqua, 'time': new Date()});
			io.sendInHome({news:{t:{game:'MINI 3 CÂY', users:user.name, bet:an, status:2}}});
		}else if (bo3) {
			an = cuoc*20;
			Mini3Cay_red.create({'uid': user.id, 'win': an, 'bet': cuoc, 'type': 4, 'kq': ketqua, 'time': new Date()});
			io.sendInHome({news:{t:{game:'MINI 3 CÂY', users:user.name, bet:an, status:2}}});
		}
		HU.updateOne({game:'mini3cay', type:cuoc, red:true}, {$inc:huUpdate}).exec();

		if (an < cuoc) {
			let betLoseToHu = cuoc - an;
			try {
				HU.findOne({ game: 'minipoker', type: cuoc, red: true }, 'bet', function(err, data) {
					if (!!data) {
