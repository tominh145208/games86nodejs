		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						TamHung_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
						 io.sendInHome({pushnohu:{title:'MEDUSA', name:user.name, bet:okHu}}); 
						nohu = true;
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'MEDUSA', users:user.name, bet:bet_win, status:2}}});
			} 
			HU.updateOne({game:'tamhung', type:bet, red:true}, {$inc:{bet:addQuy}}).exec();

			
			if (bet_win < bet) {
				let betLoseToHu = bet - bet_win;
				try {
					HU.findOne({ game: 'tamhung', type: bet, red: true }, 'bet', function (err, data) {
						if (!!data) {
