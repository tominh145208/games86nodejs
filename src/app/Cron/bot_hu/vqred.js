        ...
        Promise.all(line.map(function(selectLine) {
            ...
            .then(result => {
                ...
                result.forEach(function(line_win) {
                    if (line_win.win == 5) {
                        if (line_win.type === 5) {
                            ...
                            //io.sendInHome({pushnohu:{title:'Ai Cập', name:user.name, bet:okHu}}); 
                            nohu = true;
                ...
                if (!nohu && bet_win >= tongCuoc * 2.24) {
                    io.sendInHome({ news: { t: { game: 'Ai Cập', users: user.name, bet: bet_win, status: 2 } } });
                }
                ...
                if (bet_win < bet) {
                    ...
                    try {
                        HU.findOne({ game: 'vuongquocred', type: bet, red: true }, 'bet', function(err, data) {
                            if (!!data) {
