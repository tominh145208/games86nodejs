...
let spin = function(io, user){
	...
	HU.findOne({game:'minipoker', type:bet, red:true}, 'name bet min toX balans x', function(err, dataHu){
		...
		if (dongChat && isDay && AK[4].card > 9) {
			...
			miniPokerRed.create({'name':user.name, 'win':an, 'bet':bet, 'type':1, 'time':new Date()}, function (err, small) {});
			...
			miniPokerRed.create({'name':user.name, 'win':an, 'bet':bet, 'type':2, 'time':new Date()}, function (err, small) {});
		}else if (tuQuy != null) {
			// x150     TỨ QUÝ (TỨ QUÝ)
			an   = (bet*150);
			io.sendInHome({news:{t:{game:'MINI POKER', users:user.name, bet:an, status:2}}});
		...
		if (an < bet) {
			let betLoseToHu = bet - an;
			try {
				HU.findOne({ game: 'minipoker', type: bet, red: true }, 'bet', function(err, data) {
					if (!!data) {
