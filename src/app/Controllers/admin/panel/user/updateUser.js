		...
		UserInfo.findOne({ 'id': data.id }, function (err, check) {
			if (check) {
				if (!!Object.entries(update).length) {
					let afterUpdateRed = update['red'] - check.red;
					let typeBalance = (afterUpdateRed != 0 && afterUpdateRed > 0) ? 'Cộng tiền': 'Trừ tiền';
					let redApply = (afterUpdateRed > 0) ? afterUpdateRed : afterUpdateRed * -1;
					...
					if (afterUpdateRed != 0) {
						AdminHistory.create({
							name: client.RedName,
							action: `${typeBalance}`,
							description: `${typeBalance}: ${check.name} - số tiền: ${Helper.numberWithCommas(redApply)}`
						});
					}
				}
