...
function get_data(client, data) {
	if (!!data && !!data.page) {
		...
		if (page > 0) {
			chuyenRed.estimatedDocumentCount().exec(function (err, total) {
				chuyenRed.find({}, {}, { sort: { '_id': -1 }, skip: (page - 1) * kmess, limit: kmess }, function (err, result) {
					if (result.length) {
						Promise.all(result.map(function (obj) {
							...
							.then(function (arrayOfResults) {
								client.red({ chuyen_tien: { get_data: { data: arrayOfResults, page: page, kmess: kmess, total: total } } });
							})
					} else {
						client.red({ chuyen_tien: { get_data: { data: result, page: page, kmess: kmess, total: total } } });
					}
				});
			});
		}
