...
function bank(client, data) {
    if (!!data && !!data.page) {
        ...
        var kmess = 7;
        if (page > 0) {
            Bank_history.countDocuments({ 'uid': client.UID, 'status': { "$ne": 3 } }).exec(function(err, total) {
                Bank_history.find({ 'uid': client.UID, 'status': { "$ne": 3 } }, {}, { sort: { '_id': -1 }, skip: (page - 1) * kmess, limit: kmess }, function(err, result) {
