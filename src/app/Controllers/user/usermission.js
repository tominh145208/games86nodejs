                ...
                if (result.current * 1 >= result.totalPay) {
                    if (result.achived2) {
                        ...
                        UserInfo.findOneAndUpdate({ id: result.uid }, { $inc: { red: result.totalAchive * 0.5 >> 0 } }, function(err, user) {
                            ...
                            Messages.create({ uid: user.id, title: "ĐỔI THƯỞNG", text: "Bạn nhận được " + Helpers.numberWithCommas((result.totalAchive * 0.5) >> 0) + "  từ hoạt động nhân đôi nạp thẻ", time: new Date() });
                            client.red({ notice: { title: 'ĐỔI THƯỞNG', text: 'Đổi thưởng thành công bạn nhận được ' + ((result.totalAchive * 0.5) >> 0) + ' G !!!' }, user: { red: user.red * 1 + (result.totalAchive * 0.6 >> 0) } });
                        })
                    } else {
                        UserInfo.findOneAndUpdate({ id: result.uid }, { $inc: { red: result.totalAchive * 1 } }, function(err, user) {
                            UserMission.updateOne({ uid: client.UID, name: client.profile.name, type: type }, { $set: { achived2: true, achived: true } }).exec();
                            Messages.create({ uid: user.id, title: "ĐỔI THƯỞNG", text: "Bạn nhận được " + Helpers.numberWithCommas(result.totalAchive * 1) + "  từ hoạt động nhân đôi nạp thẻ", time: new Date() });
                            client.red({ notice: { title: 'ĐỔI THƯỞNG', text: 'Đổi thưởng thành công bạn nhận được ' + Helpers.numberWithCommas(result.totalAchive * 1) + ' G !!!' }, user: { red: user.red * 1 + result.totalAchive * 1 } });
                        })
                    }
                    ...
                    if (result.achived) {
                        ...
                        UserInfo.findOneAndUpdate({ id: result.uid }, { $inc: { red: result.totalAchive * 0.5 >> 0 } }, function(err, user) {
                            ...
                            Messages.create({ uid: user.id, title: "ĐỔI THƯỞNG", text: "Bạn nhận được " + Helpers.numberWithCommas((result.totalAchive * 0.5) >> 0) + "  từ hoạt động nhân đôi nạp thẻ", time: new Date() });
                            client.red({ notice: { title: 'ĐỔI THƯỞNG', text: 'Đổi thưởng thành công bạn nhận được ' + Helpers.numberWithCommas((result.totalAchive * 0.5) >> 0) + '  !!!' }, user: { red: user.red * 1 + (result.totalAchive * 0.5 >> 0) } });
                        })
                    }
                } else {
