                    ...
                    UserInfo.findOne({ 'id': cID }, 'red').exec(function (err, user) {
                        if (!!user) {
                            Phone.findOne({ 'uid': cID }, function (err3, checkP) {
                                if (checkP) {
                                    telegram.findOne({ 'phone': checkP.phone }, 'form', function (err3, teleCheck) {
                                        if (!!teleCheck) {
                                            OTP.findOne({ 'uid': cID, 'phone': checkP.phone }, {}, { sort: { '_id': -1 } }, function (err, data_otp) {
                                                if (!data_otp || (new Date() - Date.parse(data_otp.date)) / 1000 > 180 || data_otp.active) {
                                                    ...
                                                    try {
                                                        redT.telegram.sendMessage(teleCheck.form, '*OTP LẤY LẠI MẬT KHẨU*:  ' + otp + '', { parse_mode: 'markdown', reply_markup: { remove_keyboard: true } }).then((resp) => { }).catch((error) => { });
                                                    }catch(e) {
                                                        console.log(e);
                                                    }                                                    
                                                    //UserInfo.updateOne({ 'id': cID }, { $inc: { red: -1000 } }).exec();
                                                    client.red({ notice: { title: 'THÔNG BÁO', text: 'Mã OTP đã được gửi vào Telegram của bạn...' } });
                                                } else {
                                                    client.red({ notice: { title: 'OTP', text: 'Vui lòng kiểm tra Telegram!' } });
                                                }
                                            });
                                        } else {
                                            client.red({ notice: { title: 'THẤT BẠI', text: 'Tài khoản chưa xác thực Telegram.' } });
                                        }
                                    });
                                } else {
