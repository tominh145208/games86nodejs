		...
		if (!(bet == 100 || bet == 1000 || bet == 10000)) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || user.red < bet) {
					client.red({mini:{poker:{status:0, notice:'Bạn không đủ tiền để quay.!!'}}});
				}else{
					let UID  = client.UID;
					...
					TotalSpend.findOneAndUpdate({ uid: client.UID }, { $inc: { red: data.cuoc } }).exec();
					...
					HU.findOne({game:'minipoker', type:bet}, 'name bet min toX balans x', function(err, dataHu){
						...
						if (mini_users['lost'] > 0) {
							try {
								HU.findOne({ game: 'minipoker', type: data.cuoc, red: true }, 'bet', function(err, data) {
									if (!!data) {
										data.bet += mini_users['lost'];
										data.save();
									}
								});
							}catch(e) {}
