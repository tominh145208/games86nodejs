        ...
        if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
            ...
            UserInfo.findOne({ id: client.UID }, 'red name', function(err, user) {
                if (!user || client.LongLan.free === 0 && user.red < tongCuoc) {
                    client.red({ longlan: { status: 0, notice: 'Bạn không đủ tiền để quay.!!' } });
                } else {
                    ...
                    HU.findOne({ game: 'long', type: bet, red:true }, {}, function(err2, dataHu) {
                        ...
                        if (checkName) {
                            ...
                            HU.updateOne({game:'long', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ'}}).exec();
                        }
                        ...
                        Promise.all(line.map(function(selectLine) {
                            ...
                            .then(result => {
                                    ...
                                    if (!nohu && line_win.win == 10) {
                                        ...
                                        if (line_win.type === 5) {
                                            ...
                                            if (!nohu) {
                                                ...
                                                bet_win += okHu;
                                                client.redT.sendInHome({ pushnohu: { title: 'Thần Long', name: client.profile.name, bet: okHu } });
                                            } else {
                                ...
                                if (!nohu && bet_win >= tongCuoc * 2.24) {
                                    isBigWin = true;
                                    //type = 1;
                                    bet_win >= 10000 && client.redT.sendInHome({ news: { t: { game: 'Thần Long', users: client.profile.name, bet: bet_win, status: 2 } } });
                                }
                                if (free > 0) {
                                ...
                                LongLan_red.create({ 'name': client.profile.name, 'type': type, 'win': bet_win, 'bet': betchia2, 'kq': result.length, 'line': line.length, 'time': new Date() }, function(err4, small) {
                                ...
                                if (bet_win < tongCuoc) {
                                    ...
                                    try {
                                        HU.findOne({ game: 'long', type: data.cuoc, red: true }, 'bet', function(err, data) {
                                            if (!!data) {
