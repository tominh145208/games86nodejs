            ...
            if (client.tamhung.free > 0 && bet != client.tamhung.lastBet) {
                ...
                UserInfo.findOne({ id: client.UID }, red ? 'red name daily' : 'xu namedaily', function (err, user) {
                    if (client.tamhung.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
                        client.red({ tamhung: { status: 0, notice: 'Bạn không đủ tiền để quay.!!' } });
                    } else {
                        ...
                        HU.findOne({ game: 'tamhung', type: bet, red: true }, {}, function (err2, dataHu) {
                            ...
                            if (checkName) {
                                ...
                                HU.updateOne({game:'tamhung', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ'}}).exec();
                            }
                            ...
                            Promise.all(line.map(function (selectLine) {
                                ...
                                .then(result => {
                                    ...
                                    if (red) {
                                        ...
                                        TamHung_red.create({ 'name': client.profile.name, 'type': type, 'win': bet_win, 'bet': betchia2, 'kq': result.length, 'line': line.length, 'time': new Date() }, function (err4, small) {
                                        ...
                                        if (bet_win < tongCuoc) {
                                            ...
                                            try {
                                                HU.findOne({ game: 'tamhung', type: data.cuoc, red: true }, 'bet', function (err, data) {
                                                    if (!!data) {
