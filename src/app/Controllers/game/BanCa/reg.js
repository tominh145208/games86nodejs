	...
	if (!!data.room && !!data.balans) {
		...
		if (room === 1 || room === 2 || room === 3){
			...
			if (balans < min || balans > max) {
				client.red({notice:{title:'THẤT BẠI', text:'Vui lòng nạp ít nhất '+ Helpers.numberWithCommas(min) + "\n" +' để vào phòng!', load: false}});
			}else{
				let inGame = false;
				...
				if (inGame) {
					...
					UserInfo.findOne({id:client.UID}, 'red', function(err, user){
						if (!user || user.red < min) {
							client.red({notice:{title:'THẤT BẠI', text:'Bạn cần tối thiểu ' + Helpers.numberWithCommas(min) + ' để vào phòng.!!', load: false}});
						}else{
							if(user.red<balans) 
							{
