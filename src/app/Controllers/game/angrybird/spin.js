		...
		if (!(bet == 100 || bet == 1000 || bet == 10000)) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || user.red < bet) {
					client.red({mini:{arb:{status:0, notice:'Bạn không đủ tiền để quay.!!'}}});
				}else{
					let line_nohu = 0;
					let bet_win   = 0;
					let type      = 0;   // Loại được ăn lớn nhất trong phiên

					TotalSpend.findOneAndUpdate({ uid: client.UID }, { $inc: { red: bet } }).exec();
					...
					HU.findOne({game:'arb', type:bet, red: true}, 'name bet min toX balans x', function(err, dataHu){
						...
						if (checkName) {
							...
							celR1[1] = 4;
							celR2[1] = 4;
							HU.updateOne({game:'arb', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ', bet:quyMin}}).exec();
						}
						...
						.then(result => {
									...
									if(line_win.win == 4) {
										...
										if (heso == 100) {
											...
											if (!nohu) {
												...
												bet_win += okHu;
												client.redT.sendInHome({pushnohu:{title:'Bom Tấn', name:client.profile.name, bet:okHu}});
							...
							if (!nohu && bet_win >= bet*10) {
								...
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Bom Tấn', users:client.profile.name, bet:bet_win, status:2}}});
							}
							...
							if (tien < 0){
								...
								try {
									HU.findOne({ game: 'arb', type: data.cuoc, red: true }, 'bet', function(err, data) {
										if (!!data) {
