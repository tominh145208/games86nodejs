        ...
        if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
            ...
            UserInfo.findOne({ id: client.UID }, 'red name', function(err, user) {
                if (!user || client.DongMauAnhhung.free === 0 && user.red < tongCuoc) {
                    client.red({ DongMauAnhhung: { status: 0, notice: 'Bạn không đủ tiền để quay.!!' } });
                } else {
                    ...
                    HU.findOne({ game: 'dongmauanhhung', type: bet, red:true }, {}, function(err2, dataHu) {
                        ...
                        if (checkName) {
                            ...
                            HU.updateOne({game:'dongmauanhhung', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ'}}).exec();
                        }
                        ...
                        Promise.all(line.map(function(selectLine) {
                            ...
                            .then(result => {
                                    ...
                                    if (line_win.win == 6) {
                                        ...
                                        if (line_win.type === 5) {
                                            ...
                                            if (!nohu) {
                                                ...
                                                bet_win += okHu;
                                                client.redT.sendInHome({ pushnohu: { title: 'Dòng Máu Anh Hùng', name: client.profile.name, bet: okHu } });
                                            } else {
                                                let okHu = (dataHu.min - Math.ceil(quyHu * phe / 100)) >> 0;
                                                bet_win += okHu;
                                                client.redT.sendInHome({ pushnohu: { title: 'Dòng Máu Anh Hùng', name: client.profile.name, bet: okHu } });
                                            }
                                            huUpdate.hu += 1;
                                ...
                                if (!nohu && bet_win >= tongCuoc * 2.24) {
                                    isBigWin = true;
                                    //type = 1;
                                    bet_win >= 10000 && client.redT.sendInHome({ news: { t: { game: 'Dòng Máu Anh Hùng', users: client.profile.name, bet: bet_win, status: 2 } } });
                                }
                                if (free > 0) {
                                ...
                                DongMauAnhhung_red.create({ 'name': client.profile.name, 'type': type, 'win': bet_win, 'bet': betchia2, 'kq': result.length, 'line': line.length, 'time': new Date() }, function(err4, small) {
                                    client.DongMauAnhhung.id = small._id.toString();
                                });

                                HU.updateOne({ game: 'dongmauanhhung', type: betchia2 }, { $inc: huUpdate }).exec();
                                UserInfo.updateOne({ id: client.UID }, { $inc: uInfo }).exec();
                                DongMauAnhhung_users.updateOne({ 'uid': client.UID }, { $set: { time: new Date().getTime(), select: betchia2 }, $inc: mini_users }).exec();
                            });
                    });
                }
            });
