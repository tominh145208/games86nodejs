        ...
        if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
            ...
            UserInfo.findOne({ id: client.UID }, 'red name', function (err, user) {
                if (!user || client.Caoboi.free === 0 && user.red < tongCuoc) {
                    client.red({ Caoboi: { status: 0, notice: 'Bạn không đủ tiền để quay.!!' } });
                } else {
                    ...
                    HU.findOne({ game: 'Caoboi', type: bet, red: true }, {}, function (err2, dataHu) {
                        ...
                        if (checkName) {
                            ...
                            HU.updateOne({game:'Caoboi', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ'}}).exec();
                        }
                        ...
                        Promise.all(line.map(function (selectLine) {
                            ...
                            .then(result => {
                                ...
                                Caoboi_red.create({ 'name': client.profile.name, 'type': type, 'win': bet_win, 'bet': betchia2, 'kq': result.length, 'line': line.length, 'time': new Date() }, function (err4, small) {
                                    client.red({ Caoboi: { lmao: true } });
                                ...
                                HU.updateOne({ game: 'Caoboi', type: betchia2 }, { $inc: huUpdate }).exec();
