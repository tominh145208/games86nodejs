            ...
            if (client.SieuXe.free > 0 && bet != client.SieuXe.lastBet) {
                ...
                UserInfo.findOne({ id: client.UID }, red ? 'red name' : 'xu name', function (err, user) {
                    if (client.SieuXe.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
                        client.red({ sieuxe: { status: 0, notice: 'Bạn không đủ tiền để quay.!!' } });
                    } else {
                        let config = Helpers.getConfig('sieuxe');
                        let phe = red ? 2 : 4; // Phế
                        ...
                        HU.findOne({ game: 'sieu', type: bet, red: true }, {}, function (err2, dataHu) {
                            ...
                            if (checkName) {
                                ...
                                HU.updateOne({game:'sieu', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ'}}).exec();
                            }
                            ...
                            Promise.all(line.map(function (selectLine) {
                                ...
                                .then(result => {
                                    ...
                                    if (red) {
                                        ...
                                        SieuXe_red.create({ 'name': client.profile.name, 'type': type, 'win': bet_win, 'bet': betchia2, 'kq': result.length, 'line': line.length, 'time': new Date() }, function (err4, small) {
                                    ...
                                    if (bet_win < tongCuoc) {
                                        ...
                                        try {
                                            HU.findOne({ game: 'sieu', type: data.cuoc, red: true }, 'bet', function (err, data) {
                                                if (!!data) {
