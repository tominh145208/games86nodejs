        ...
        if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
            ...
            UserInfo.findOne({ id: client.UID }, red ? 'red name daily' : 'xu name daily', function (err, user) {
                if (client.Daohaitac.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
                    client.red({ daohaitac: { status: 0, notice: 'Bạn không đủ tiền để quay.!!' } });
                } else {
                    let config = Helpers.getConfig('daohaitac');
                    let phe = red ? 2 : 4; // Phế
                    ...
                    HU.findOne({ game: 'daohaitac', type: bet, red: true }, {}, function (err2, dataHu) {
                        ...
                        if (checkName) {
                            ...
                            HU.updateOne({game:'daohaitac', type:bet, red: true}, {$set:{name: dataHu.name + ': Đã Nổ'}}).exec();
                        }
                        ...
                        Promise.all(line.map(function (selectLine) {
                            ...
                            .then(result => {
                                ...
                                if (red) {
                                    ...
                                    Daohaitac_red.create({ 'name': client.profile.name, 'type': type, 'win': bet_win, 'bet': betchia2, 'kq': result.length, 'line': line.length, 'time': new Date() }, function (err4, small) {
                                ...
                                HU.updateOne({ game: 'daohaitac', type: betchia2, red: red }, { $inc: huUpdate }).exec();
