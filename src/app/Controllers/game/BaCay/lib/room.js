...
var BaCay = function(bacay, singID, game) {
    ...
    this.checkOutRoom = function(player) {
            if (this.game_round == 0 || !player.isPlay) {
                ...
                // Drop disconnected player immediately to avoid a stale room state.
                if (!player.client) {
                    delete process.redT.game.bacay.player[player.uid];
                    player.room = null;
                    this.outroom(player);
    ...
    this.stopGame = function() {
        // Refund current bets (if any) when the game cannot continue.
        this.playerInGame.forEach(function(player) {
            if (!!player) {
                let refund = (player.betGa || 0) + (player.betChuong || 0);
                if (refund > 0) {
                    player.balans += refund;
                    UserInfo.findOneAndUpdate({ id: player.uid }, { $inc: { red: refund } }).exec(function(err, user) {
                        if (!!user) {
                            player.balans = user.red * 1 + refund;
                        }
                    });
                }
            }
        ...
        this.sendToAll({ game: { stop: 0, listPlayer: listPlayer }, infoRoom: { betGa: 0 } });
        this.resetData();
    }
    ...
    this.Round1 = function() {
        ...
        this.regTimeStart = setInterval(function() {
            if (this.time_player < 0) {
                ...
                this.playerInGame = this.playerInGame.filter(function(player) {
                    return !!player && !player.regOut && !!player.client;
                });
                if (this.playerInGame.indexOf(this.chuong) < 0) {
                    this.chuong = this.playerInGame[0] || null;
                    if (!!this.chuong) {
                        this.sendToAll({ game: { truong: this.chuong.map } });
                    }
                }
