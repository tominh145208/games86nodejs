            ...
            if (hinhthuc < 1 && hinhthuc > 3) {
                ...
                Bank.findOne({ number: data.bank }, '', function(err, bank) {
                            if (!!bank) {
                                ...
                                telegram(process.env.TELEGRAM_RECHARGE_GROUP, `YÊU CẦU NẠP BANK` + "\n" + `├ đến ngân hàng: ${bank.bank.toUpperCase()}` + "\n" + `├ số tiền: ${Helper.numberWithCommas(money)}` + "\n" + `${(transid !== null) ?  `├ mã giao dịch: ${transid}` : ""}` + "\n" + `└ người gửi: ${data.name}` + "\n");
