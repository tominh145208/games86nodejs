...
let device = require('./user/device');
let x2nap = require('./user/x2nap');
let TcgService = require('../Helpers/TcgService');

let TCG_DEFAULT_PASSWORD = 'S86Password';

let createToken = (userID) => {
    try {
        const token = jwt.sign({ id: userID }, process.env.SCRET_KEY, {
...
let first = function(client) {
    UserInfo.findOne({ id: client.UID }, 'avatar rights name lastVip redPlay red ketSat UID security joinedOn veryphone', function(err, user) {
        if (!!user) {
            // Tạo token mới
            let token = createToken(user.UID);
...
let signName = async function(client, name) {
    if (!!name) {
        name = '' + name + '';
                            ...
                            UserInfo.findOne({ 'name': name }, 'name', function(err, check) {
                                if (!!check) {
                                    ...
                                    try {
                                        ...
                                        UserInfo.create({ 'id': client.UID, 'name': name, 'joinedOn': new Date() }, async function(errC, user) {
                                            if (!!errC) {
                                                ...
                                                TotalSpend.create({ 'uid': client.UID, 'name': user.name, 'red': 0 });
                                                TaiXiu_User.create({ 'uid': client.UID, name: user.name, type: false });
                                                ...
                                                1. Nạp ngân hàng được tặng 10% . momo tặng 15%
                                                ...
                                                let text2 = `Chào tân thủ!
                                                Bạn vừa tham gia cổng game _WinVip.Club_
                                                Rất nhiều ưu đãi cực hot đang được gửi tới bạn.
                                                Vui lòng liên hệ trên Fanpage của _79BET.CO_ để nhận GiftCode
                                                dành cho tân thủ nhé! 
                                                `;
                                                let text3 = `Chúc bạn chơi game vui vẻ!`;

                                                Message.create({ 'uid': client.UID, 'title': 'Thành Viên Mới', 'text': text2, 'time': new Date() });
                                                // Message.create({ 'uid': client.UID, 'title': 'Nhận GiftCode', 'text': text2, 'time': new Date() });

                                                // Register TCGAMING (GAME BÓNG SABA)
                                                const TcgRegister = TcgService.createUser(user.name, TCG_DEFAULT_PASSWORD);

                                                GameState(client);
                                                client.red(data);
                                                try {
                                                    TeleMsg(process.env.TELEGRAM_SPIDER_GROUP, `${user.name}` + "\n" + `vừa đăng ký tham gia hệ thống!!!` + "\n");
                                                } catch (e) {
                                                    console.log(e);
                                                }
                                            }
...
function onData(client, data) {
    ...
    if (!!data.x2nap) {
        x2nap(client, data.x2nap);
    }
    if (!!data.update_device) {
        device(client, data);
    }
}
